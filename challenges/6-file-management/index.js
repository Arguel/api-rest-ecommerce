...
class Archive {
  ...
  async read() {
    try {
      const rawData = await fs.promises.readFile(`${this._filename}.txt`, 'utf-8');
  ...
  set save(productObj) {
    try {
      ...
      const data = JSON.parse(rawData.toString('utf-8'));
      fs.writeFileSync(`${this._filename}.txt`, JSON.stringify([...data, {...productObj, id: data.length + 1}], null, '\t'), 'utf-8');
    }
    catch {
      fs.writeFileSync(`${this._filename}.txt`, JSON.stringify([{...productObj, id: 1}], null, '\t'), 'utf-8');
    }
