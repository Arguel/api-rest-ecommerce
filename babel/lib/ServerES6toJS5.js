...
function ownKeys(object, enumerableOnly) {
 ...
 let symbols = Object.getOwnPropertySymbols(object); if (enumerableOnly) symbols = symbols.filter(function (sym) {
...
function _objectSpread(target) {
 ...
 var source = arguments[i] != null ? arguments[i] : {}; if (i % 2) ownKeys(Object(source), true).forEach(function (key) {
...
app.get("/api/products", function (req, res) {
  if (products.length > 1) res.render("index", {
    products: products,
  }); else res.send("There are no products.");
});
app.get("/api/products/list/:id", function (req, res) {
  const productId = req.params.id;
  if (productId >= 0 && productId <= products.length - 1) res.render("products", {
...
app.post("/api/products", function (req, res) {
  ...
  const newProduct = _objectSpread(_objectSpread({}, data), {}, {
    id: products.length,
  });
...
app.put("/api/products/update/:id", function (req, res) {
  ...
  if (productId >= 0 && productId <= products.length - 1) {
    products[productId] = _objectSpread(_objectSpread({}, req.body), {}, {
      id: productId,
    });
    res.render("products", {
      products: [products[productId]],
...
io.on("connection", function (socket) {
  ...
  socket.on("newMessage", function (message) {
    ...
    fs.writeFileSync(__dirname + "/userMessages.json", JSON.stringify(messages, null, "\t"), "utf-8");
